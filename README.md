...
- **Cross-sectional momentum (beat the index out-of-sample, but not a fair bar):** each month, hold the ten strongest names by their twelve-month return skipping the most recent month, weight them by inverse volatility, and step to cash when the index is below a buffered long-term average. In-sample it scored an excess Sharpe of 0.98 against 0.24, with every cell of the sensitivity grid staying positive. The single holdout run came back at 0.59 against the market's 0.41: the edge decayed out-of-sample, as edges do, but stayed above the market after full costs. The market, though, turns out to be the wrong bar. A portfolio of the same hundred-odd names picked with no information at all scored 0.87 to 0.91 over that window, so beating the index by 0.18 is what survivorship alone pays: simply holding the whole universe would have done better than the strategy. Comparing it instead against portfolios built the same way but holding the same ten names picked at random, the ranking is worth between +0.06 and +0.24 depending on whether the regime filter is applied to both sides — positive, but seven of twenty random draws still beat it, so it sits inside the noise. This window cannot settle whether the ranking adds anything. Most of the shortfall against the hundred-name portfolio is diversification and the regime filter, not the picking. One design element failed along the way, a hard rule to flatten the book permanently on a deep drawdown that tripped at the very bottom of the 2020 crash and would have locked in the loss, so that rule was dropped. It now runs as a live bot in the tournament, which is its real forward test.
...
Every figure here is an upper bound. The universe is the set of names that are liquid today, held fixed across the whole history, so a company that would have ranked well in 2008 and later collapsed can never be picked, which flatters the long-history numbers by an unknowable amount. The closest thing to a survivor decayed from an in-sample excess Sharpe of 0.98 to 0.59 out-of-sample, and that 0.59 still trails a no-information portfolio of the same names. [METHODOLOGY.md](METHODOLOGY.md) states these biases in full.

## Tests

```bash
npm test          # 652 tests, runs with node --test
```
