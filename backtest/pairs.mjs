  ...
  for (; gi < master.length; gi++) {
    ...
    if (pending && pendingPairs) {
      ...
      for (const s of universe) {
        ...
        if (!(px > 0)) continue;
        const cur = posQty(s);
        if (cur !== 0 && !wantedPair[s]) { moveTo(s, cur, 0, px, `Closed the ${s} leg — its pair is no longer selected, so the position was exited.`); posPair[s] = null; }
      }
    ...
    for (const p of activePairs) {
      ...
      if (z != null) {
        if (p.dir === 0) {
          if (z >= entryZ) p.dir = -1;        // spread rich -> short A, long B
          else if (z <= -entryZ) p.dir = 1;   // spread cheap -> long A, short B
        } else {
