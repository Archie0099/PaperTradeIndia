...
test('"Load my positions" honours a copied leg\'s stamped expiryMs (cyc legs are not ~7-day — regression)', () => {
  const dom = setupDom();
  const app = mount(dom, { symbol: 'NIFTY' });
  // An Auto-Pilot-copied option leg exactly as fno.mjs stamps it: a synthetic
  // "cyc0" expiry STRING (unparseable — the old code silently fell back to
  // "now + 7 days") plus the REAL timestamp on expiryMs (~45 days out). The
  // wrong 7-day horizon distorted the recovered IV and every Greek/breakeven
  // of the loaded strategy. positions.js already preferred expiryMs; this
  // locks the same seam in the builder's loader.
  app.engine.placeOrder({
    instrument: { kind: 'OPT', symbol: 'NIFTY', expiry: 'cyc0', strike: 23000, optType: 'PE', lotSize: 75, underlyingPrice: 23500, expiryMs: Date.now() + 45 * 864e5, iv: 0.18 },
    side: 'SELL',
    orderType: 'MARKET',
    lots: 1,
    price: 230,
  ...
  dom.fire(dom.$('#btn-load-positions'), 'click');
  const rs = rows(dom);
  assert.equal(rs.length, 1, 'the copied leg loads');
