...
async function autopilotTick(app, { force = false } = {}) {
  ...
  try {
    ...
    const mirror = detail && detail.mirror;
    if (!mirror) {
      statusMsg = `${target.name} can't be copied yet.`;
      renderIfVisible(app);
    ...
    // A held name the bot does NOT target (e.g. a position was manually opened while the
    // Auto-Pilot was on) means we are NOT truly in sync — don't short-circuit, drive it flat.
    const targetKeys = new Set((mirror.positions || []).map((p) => p.key));
    const hasForeign = currentPositions(app).some((c) => !targetKeys.has(c.key));
    if (!force && sig === getCfg().lastSig && !hasForeign) {
      // The bot hasn't changed its portfolio since our last copy — nothing to do.
      patchCfg({ followId: target.id }); // record who we follow; never re-write lastSig here
    ...
    // clobber a setting changed during this tick's awaits.
    const synced = results.every((r) => r.status === 'FILLED' && !r.capped);
    patchCfg({ followId: target.id, lastSig: synced ? sig : null, lastSyncTs: Date.now() });
...
function renderApDetail(app) {
  ...
  for (const o of eng.state.orders) {
    if (o.status === 'FILLED' && o.closing && Number.isFinite(o.realised) && o.instrument) {
  ...
  const fills = eng.state.orders.filter((o) => o.status === 'FILLED');
  // A SEEDED account ("Reflect this in my account") carries the multi-year walk-forward equity curve
  // but NO orders (buildSeededState sets orders:[]) — so a bare "No trades yet" would misread a ₹13cr
  // seeded track record sitting right beneath its own 16-year curve. Detect it (a curve spanning > a
  // month with zero fills = a seeded simulated track record) and explain honestly.
  const seededTrack = curve.length > 1 && spanOf(curve) > 31 * 864e5 && !fills.length;
  ...
  if (!fills.length) {
    ...
    for (const o of fills.slice(0, 100)) {
      tb.append(el('tr', {}, [
        el('td', {}, new Date(o.fillTs || o.ts).toLocaleDateString('en-IN')),
  ...
  if (ap && Array.isArray(ap.followedTimeline) && ap.followedTimeline.length) {
    ...
    for (const f of ap.followedTimeline) ul.append(el('li', {}, [el('span', { class: 'muted' }, `${new Date(f.t).toLocaleDateString('en-IN')} → `), f.name]));
    root.append(ul);
  }
}
