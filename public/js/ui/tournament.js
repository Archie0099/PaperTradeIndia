...
function renderTrades(box, d) {
  ...
  trades.slice().reverse().forEach((tr, i) => {
    // All displayed market dates are rendered IN IST ({ timeZone }): 'en-IN' alone sets
    // only the FORMAT — a viewer west of UTC would otherwise see NSE's 09:15-IST bars
    // dated one day EARLIER than the same bot's IST chart axis (the same option is
    // applied at every date/time render site in this file + autopilot/orders).
    const dt = Number.isFinite(tr.t) ? new Date(tr.t).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: '2-digit', timeZone: 'Asia/Kolkata' }) : '—';
...
function render(app, data) {
  ...
  if (meta) {
    const deployed = data.deployedAt ? new Date(data.deployedAt).toLocaleDateString('en-IN', { timeZone: 'Asia/Kolkata' }) : '—';
...
function renderBotPage(body, d, row) {
  ...
  body.append(el('div', { class: 'muted', style: 'margin-bottom:10px' },
    `${d.symbol} · ${d.kind}${intervalTag}${d.gen > 0 ? ` · evolved (gen ${d.gen})` : ''}${d.deployAt ? ` · deployed ${new Date(d.deployAt).toLocaleDateString('en-IN', { timeZone: 'Asia/Kolkata' })}` : ''}`));
  ...
  if (m.sharpePostProxy != null && m.sharpePostProxy !== m.sharpe) {
    stats.append(stat('Sharpe from proxy start', String(m.sharpePostProxy)));
  }
...
function renderDecision(body, dec) {
  ...
  const when = Number.isFinite(dec.t) ? new Date(dec.t).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: '2-digit', timeZone: 'Asia/Kolkata' }) : '—';
