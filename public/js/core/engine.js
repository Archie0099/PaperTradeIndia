...
class Engine {
  ...
  importJson(text) {
    ...
    // Each last price must be a finite number — a non-numeric value would flow
    // through unrealisedFor()/equity() as NaN and brick every render.
    for (const key of Object.keys(merged.lastPrices)) {
      const v = merged.lastPrices[key];
      if (typeof v !== 'number' || !Number.isFinite(v)) {
        throw new Error(`Invalid portfolio file: lastPrices["${key}"] must be a finite number.`);
      }
    }
    ...
    for (const o of merged.orders) {
      ...
      if (o.status === 'PENDING') {
        ...
        if (typeof o.limitPrice !== 'number' || !Number.isFinite(o.limitPrice)) {
          throw new Error('Invalid portfolio file: a pending order has a non-finite limit price.');
        }
