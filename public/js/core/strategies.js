...
function analyse(legs, spot, opts = {}) {
  ...
  // A FUTURE's breakeven sits at its ENTRY price (its payoff crosses zero there),
  // which is not a strike — so it must anchor the analysis window too. Without it,
  // a future entered far above 3x the current spot (a crashed underlying) had its
  // genuine breakeven silently dropped from the authoritative stats (the chart
  // window is a sketch, but breakevens/max-P&L must be exact).
  const futEntries = norm.filter((l) => l.type === 'FUT').map((l) => l.premium).filter((p) => Number.isFinite(p) && p > 0);
  const far = Math.max(spot, ...(strikes.length ? strikes : [spot]), ...futEntries) * 3 + 10;
  const points = [...new Set([0, spot, far, ...strikes, ...futEntries])].filter((x) => x >= 0).sort((a, b) => a - b);
