...
app.pollQuotes = async function pollQuotes() {
  ...
  if (q) $('#chart-ltp').innerHTML = ltpHeaderHtml(q, app.state.activeCandles, app.state.timeframe);
  else $('#chart-ltp').textContent = '';
...
function main() {
  ...
  initTheme(() => {
    if (app.state.activeCandles) {
      const tf = TIMEFRAMES[app.state.timeframe] || TIMEFRAMES['1M'];
      const aq = app.state.quotes[app.state.activeSymbol];
      const ahc = aq ? headerChange(aq, app.state.activeCandles, app.state.timeframe) : null;
      drawLineChart($('#price-chart'), app.state.activeCandles, { title: app.state.activeSymbol, interval: tf.interval, changeSign: ahc ? ahc.changePct : null });
    }
