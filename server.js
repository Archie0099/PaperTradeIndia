...
// `add` one moment later gets 429. It also told a legitimate user who reloaded and clicked again
// "Too fast", a second unrelated-looking failure. Refusing before the limiter costs nothing: the
// check is a string compare against an already-cached board.
//
// ★ WHAT THIS DOES *NOT* FIX, so nobody reads the above as more than it is: the generic
// window-holding DoS is still open. `remove` and `add` stamp the window in `tournRateLimit` and
// only then decide the request was bad — so a script POSTing `/api/tournament/remove?id=nope`
// every ~600ms still holds the shared window continuously and makes every legitimate click race
// for a gap. That predates this change and is part of the open "lock the mutating POSTs"
// decision; the reorder closes it for RESET only, which is the door with the irreversible blast
// radius behind it.
const resetConfirm = resetConfirmMiddleware(() => tournament);
...
(async () => {
  try {
    ...
    // ...then poll for a new daily bar. Each interval is RE-ENTRANCY guarded: if a
    // previous tick is still in flight when the timer fires again (a slow upstream,
    // or the once-a-day standings recompute), firing another on top would stack
    // never-finished ticks — each pinning a pending promise and an open socket on
    // this long-lived process. Skip the beat instead; the next one catches up (and
    ...
    setInterval(async () => {
      ...
      intradayBusy = true;
      try { await t.tickIntraday(); } catch { /* no new bars */ } finally { intradayBusy = false; }
    }, 10 * 60 * 1000);
